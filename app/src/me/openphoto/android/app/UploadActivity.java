...
public class UploadActivity extends Activity implements OnClickListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (getIntent() != null
			...
			setSelectedImageUri((Uri) extras.getParcelable(Intent.EXTRA_STREAM));
				...
				&& getIntent().hasExtra(EXTRA_PENDING_UPLOAD_URI)) {
			Uri uri = getIntent().getParcelableExtra(EXTRA_PENDING_UPLOAD_URI);
			PhotoUpload pendingUpload = new UploadsProviderAccessor(this)
			...
			((EditText) findViewById(R.id.edit_title)).setText(pendingUpload
					.getMetaData().getTitle());
			((EditText) findViewById(R.id.edit_description))
					.setText(pendingUpload.getMetaData().getDescription());
			((EditText) findViewById(R.id.edit_tags)).setText(pendingUpload
					.getMetaData().getTags());
	...
	private void setSelectedImageUri(Uri imageUri) {
		mUploadImageFile = new File(ImageUtils.getRealPathFromURI(this,
				imageUri));
		setSelectedImageFile(mUploadImageFile);
	}
	...
	private void setSelectedImageFile(File imageFile) {
		...
		previewImage.setImageBitmap(ImageUtils
				.decodeFile(mUploadImageFile, 200));
	}
	...
	private void startUpload(File uploadFile) {
		...
		metaData.setTitle(((EditText) findViewById(R.id.edit_title)).getText()
				.toString());
		metaData.setDescription(((EditText) findViewById(R.id.edit_description))
				.getText().toString());
		metaData.setTags(((EditText) findViewById(R.id.edit_tags)).getText()
				.toString());
