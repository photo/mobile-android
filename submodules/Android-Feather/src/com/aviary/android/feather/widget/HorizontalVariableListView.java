...
public class HorizontalVariableListView extends HorizontalListView implements OnGestureListener, FlingRunnableView {
   ...
   public interface OnItemClickedListener {
	...
	protected int mSelectedPositionInt = INVALID_POSITION;
	...
	public void setSelectedPosition( int position, boolean fireEvent ) {
		...
		mSelectedPositionInt = position;
		
		if( position == INVALID_POSITION ) {
			setSelectedItem( null, INVALID_POSITION );

			if ( mOnItemSelected != null && fireEvent ) {
			...
			if( null != child ) {
				setSelectedItem( child, position );
				
				if ( mOnItemSelected != null && fireEvent ) {
					mOnItemSelected.onItemSelected( this, child, position, mAdapter.getItemId( position ) );
				}
			}
		}
	}
	...
	private void fillListLeft( int positionX, int leftEdge ) {
		...
		while ( ( leftEdge - positionX ) > mLeftEdge && mLeftViewIndex >= 0 ) {
			final boolean selected = mSelectedPositionInt == mLeftViewIndex;
	...
	private void fillListRight( int positionX, int rightEdge ) {
		...
		while ( ( rightEdge - positionX ) < viewWidth || firstChild ) {
			...
			final boolean selected = mSelectedPositionInt == mRightViewIndex;
	...
	protected void layoutChild( View child, int left, int right, int childHeight ) {
		...
		// Log.i( LOG_TAG, "layoutChild. height: " + mHeight + ", child.height: " + childHeight );
	...
	private void onItemClick( View child, int position ) {
		...
		if( clickValid ) {
			...
			if ( position != mSelectedPositionInt ) {
	...
	protected void setSelectedItem( final View newView, final int position ) { 
		
		if( mSelectedPositionView != null ) {
			mSelectedPositionView.setSelected( false );
		}
